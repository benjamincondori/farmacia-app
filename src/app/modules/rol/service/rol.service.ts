...
export class RolService {
  ...
  getRolById(id: number): Observable<Rol> {
    ...
    if (token) {
      return this.http
        .get<Rol>(`${this.apiUrl}/${id}`, httpOptions(token))
  ...
  updateRol(id: number, rol: Rol): Observable<Rol> {
    ...
    if (token) {
      // Verificar que todos los campos sean válidos antes de enviar
      if (!rol.nombre_Rol || !rol.descripcion) {
        console.error('Faltan datos necesarios para actualizar el rol');
        return of({} as Rol); // Retorna un objeto vacío si falta algún dato
      }

      return this.http
        .put<Rol>(`${this.apiUrl}/${id}`, rol, httpOptions(token))
        .pipe(catchError(this.handleError('updateRol', rol)));
    } else {
      console.error('No hay token disponible');
      return of({} as Rol); // Retorna un objeto vacío si no hay token
    }
  ...
  createRolUsuario(
    rolusuario: RolPermisoUsuario
  ): Observable<RolPermisoUsuario> {
    ...
    if (token) {
      ...
      return this.http
        ...
        .pipe(
          catchError(this.handleError('addRolPermisoUsuarios', rolusuario))
        );
    } else {
      console.error('No hay token disponible');
  ...
  getRolUsuarioById(id: number): Observable<RolPermisoUsuario> {
    ...
    if (token) {
      return this.http
        ...
        .pipe(
          catchError(
            this.handleError('getRolUsuarioById', {} as RolPermisoUsuario)
          )
        );
    } else {
  ...
  deleteRolUsuario(id_rolusuario?: number): Observable<any> {
    ...
    if (token) {
      ...
      return this.http
        ...
        .pipe(
          catchError(this.handleError('addRolPermisoUsuarios', id_rolusuario))
        );
    } else {
      console.error('No hay token disponible');
      return of({} as RolPermisoUsuario); // Retorna un objeto vacío si no hay token
