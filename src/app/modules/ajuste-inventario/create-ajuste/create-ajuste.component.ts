...
@Component({
  ...
  styleUrl: './create-ajuste.component.css',
})
export class CreateAjusteComponent {
  ...
  data: any = {
    ...
    subtitle: 'Crear ajuste de inventario',
  ...
  onAlmacenChange(event: Event): void {
    ...
    this.selectedAlmacenId = selectElement.value ? Number(selectElement.value) : null;
  ...
  onProductoChange(event: Event): void {
    const selectElement = event.target as HTMLSelectElement;
    this.selectedProductoId = selectElement.value ? Number(selectElement.value) : null;
  }
